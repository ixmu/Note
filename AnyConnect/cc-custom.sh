...
if [ $? -eq 0 ]; then
  ...
  apt-get install -y curl wget netcat openssl gnutls-bin xz-utils iptables htop vim
fi
...
if [ "$VER" == "amd64" ]; then
  rm -rf /etc/vlmcs
  wget --no-check-certificate -4 -qO /tmp/vlmcs.tar.xz "https://raw.githubusercontent.com/MoeClub/Note/master/AnyConnect/vlmcsd/vlmcsd_${VER}.tar"
...
# ocserv
rm -rf /etc/ocserv
wget --no-check-certificate -4 -qO /tmp/ocserv_bin.tar.xz "https://github.com/robvanoostenrijk/openconnect-server-static/releases/download/v1.2.4/openconnect-server-linux-${VER}.tar.xz"
