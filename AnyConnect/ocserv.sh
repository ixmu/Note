...
if [ $? -eq 0 ]; then
  ...
  apt-get install -y curl wget openssl gnutls-bin xz-utils ncat
fi
...
if [ "$VER" == "amd64" ]; then
  ...
  tar --overwrite -xzf /tmp/vlmcs.tar -C /
...
# ocserv
rm -rf /etc/ocserv
wget --no-check-certificate -4 -qO /tmp/ocserv_bin.tar.gz "https://raw.githubusercontent.com/ixmu/Note/master/AnyConnect/build/ocserv_${VER}_v1.2.4.tar.gz"
