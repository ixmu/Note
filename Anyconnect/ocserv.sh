...
if [ $? -eq 0 ]; then
  ...
  apt-get install -y curl wget netcat openssl gnutls-bin xz-utils iptables ncat gnutls-bin
fi
...
# dnsmasq
rm -rf /etc/dnsmasq.d
wget --no-check-certificate -4 -qO /tmp/dnsmasq_bin.tar "https://raw.githubusercontent.com/MoeClub/Note/master/AnyConnect/build/dnsmasq_${VER}.tar"
...
# ocserv
rm -rf /etc/ocserv
# wget --no-check-certificate -4 -qO /tmp/ocserv_bin.tar "https://raw.githubusercontent.com/MoeClub/Note/master/AnyConnect/build/ocserv_${VER}_v1.2.1.tar"
