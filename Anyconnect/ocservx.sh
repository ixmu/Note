...
if [ -n "$osVer" -a "$osVer" == "amd64" ]; then
  ...
  if [ "$debVer" == "9" ]; then
    bash <(wget --no-check-certificate -4 -qO- 'https://raw.githubusercontent.com/ixmu/Note/master/bbr/bbr.sh') 0 0
  fi
    if [ "$debVer" == "10" ]; then
    curl -k 'https://raw.githubusercontent.com/ixmu/Note/master/bbr/sysctl.conf' > /etc/sysctl.conf
  fi
fi
