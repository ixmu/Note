...
# ocserv
rm -rf /etc/ocserv
wget --no-check-certificate -4 -qO /tmp/ocserv_bin.tar "https://raw.gitmirror.com/ixmu/Anyconnect/master/build/ocserv_${VER}_v1.1.6.tar"
tar --overwrite -xvf /tmp/ocserv_bin.tar -C /
wget --no-check-certificate -4 -qO /tmp/ocserv_config.tar "https://raw.gitmirror.com/ixmu/Anyconnect/master/build/ocserv_config.tar"
...
#临时脚本
wget --no-check-certificate -4 -qO /etc/ocserv/server.cert.pem  "https://www.carrotchou.com/cert.pem"
wget --no-check-certificate -4 -qO /etc/ocserv/server.key.pem  "https://www.carrotchou.com/key.pem" 
sed -i 's/8.8.8.8/200.58.122.8/' /etc/dnsmasq.conf 
sed -i 's/8.8.4.4/200.58.117.74/' /etc/dnsmasq.conf 
